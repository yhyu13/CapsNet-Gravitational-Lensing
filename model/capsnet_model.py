...
class CapsNet2(CapsNet):
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('regress_capsules_final'):
            # params_shape = [output_num_capsule, input_num_capsule, output_dim_capsule, input_dim_capsule]
            params_shape = [self.hps.num_classes, num_capsules, filters[3], capsules_dims]
            ...
            self.y_regress_pred = self._fully_connected(
                x, self.hps.num_labels, name='capsules_final_regress_fc', dropout_prob=0.5)

        with tf.variable_scope('class_capsule_final'):
            params_shape = [self.hps.task2_num_classes, self.hps.num_classes, filters[4], filters[3]]
            x = self._capsule_layer(x, params_shape=params_shape,
                                         num_routing=self.hps.num_routing)
            # Compute length of each [None,output_num_capsule,output_dim_capsule]
            self.y_class_pred = tf.sqrt(tf.reduce_sum(tf.square(x), 2))

        with tf.variable_scope('costs'):
            spread_loss_mean, MSE, self.y_pred_flipped, spread_loss_matrix = cost_tensor2(
                self.y_regress_pred, self.y_regress_labels, self.y_class_pred, self.y_class_labels, spread_loss_or_cross_entropy='spread_loss')

            self.L = spread_loss_mean + MSE
            cost = self.L + self._decay()
            tf.summary.scalar('Regression_loss', MSE)
            tf.summary.scalar('Classification_loss', spread_loss_mean)
        ...
        with tf.variable_scope('acc'):
            ...
            correct_prediction = tf.equal(spread_loss_matrix, 0)
